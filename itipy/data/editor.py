...
class FixDateEditor(Editor):
    """
    ADD 1920 FITS TIMESYS='UTC'
    """
    def call(self, s_map, **kwargs):
        hdr = s_map.meta
        if hdr.get('DATE-OBS', '').endswith('Z'):
            hdr['DATE-OBS'] = hdr['DATE-OBS'][:-1]   # strip 'Z'

        hdr.setdefault('TIMESYS', 'UTC')

        return Map(np.copy(s_map.data), hdr)

class Prep1920Editor(Editor):
    """
    Preparation for 1920 KSO data
    """

    def __init__(self, default_exptime=1.0):
        self.default_exptime = default_exptime

    def call(self, filepath, **kwargs):
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            hdul = fits.open(filepath, mode="readonly")
            data  = hdul[0].data.astype(np.float32)
            header = hdul[0].header.copy()   # 不改原文件
            hdul.close()

        header['CUNIT1'] = 'arcsec'
        header['CUNIT2'] = 'arcsec'

        raw_date = header.get('DATE-OBS') or header.get('DATE_OBS')
        if raw_date:
            try:
                datetime.strptime(raw_date, '%Y-%m-%dT%H:%M:%SZ')
            except ValueError:
                try:
                    dt = datetime.strptime(raw_date, '%m-%d-%YT%H:%M')
                    header['DATE-OBS'] = dt.strftime('%Y-%m-%dT%H:%M:%SZ')
                except ValueError:
                    print(f"[Prep1920Editor] DATE-OBS: {raw_date}")

        if 'EXPTIME' not in header and 'EXPOSURE' not in header:
            header['EXPTIME'] = self.default_exptime
